...
Before connecting, request a WebSocket token from the widget endpoint:

```
POST /api/v1/widget/ws-token
```

This endpoint uses widget (origin-based) authentication: pass your public `X-App-Id`, a per-visitor `X-Visitor-Id`, and the request `Origin`. No API key is required — the server validates that the origin matches the domain registered for the application.

### cURL Example

```bash
curl -X POST https://api.deliverychat.online/api/v1/widget/ws-token \
...
```
wss://api.deliverychat.online/api/v1/ws?token=eyJhbGci...
```
...
const ws = new WebSocket(
  `wss://api.deliverychat.online/api/v1/ws?token=${token}`
);
...
| `contentFormat` | string | No | `"plain"` (default) or `"lexical"`. See [Message Formats](/v1/rest-api/MessageFormats). |
...
function connect() {
  const ws = new WebSocket(`wss://api.deliverychat.online/api/v1/ws?token=${token}`);
