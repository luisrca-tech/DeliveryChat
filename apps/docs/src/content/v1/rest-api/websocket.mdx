...
Open a WebSocket connection using the token and app ID as query parameters:

```
wss://api.deliverychat.online/v1/ws?token=eyJhbGci...&appId=your-app-id
```
...
const ws = new WebSocket(
  `wss://api.deliverychat.online/v1/ws?token=${token}&appId=${appId}`
);
...
<Callout type="info" title="Why query parameters?">
  WebSocket does not support custom headers during the handshake in browsers. The token and app ID are passed as query parameters instead.
</Callout>
...
Error codes: `PARSE_ERROR`, `VALIDATION_ERROR`, `FORBIDDEN`, `UNAUTHORIZED`.
...
function connect() {
  const ws = new WebSocket(`wss://api.deliverychat.online/v1/ws?token=${token}&appId=${appId}`);
