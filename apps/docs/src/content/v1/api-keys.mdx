...
You do **not** need an API key for the simple iframe embed—the iframe uses only your application ID (appId), which is public.

## When Do You Need Them?

| Integration | API Key Required? |
|-------------|-------------------|
| **iframe** | No — use appId only |
| **SDK** | Yes |
| **REST API** | Yes |
...
For **iframe**, you only need the application ID (appId). No API key is used.

For **SDK or REST API**, include the key in your requests. Example:

```typescript
// SDK or REST API call
const response = await fetch("https://api.yourcompany.deliverychat.com/v1/public/visitors", {
  headers: {
...
<QuickLinks
  links={[
    ...
    },
    {
      ...
      description: "Choose your integration method (iframe, SDK, REST)"
    },
    {
