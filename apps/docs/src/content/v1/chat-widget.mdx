...
<script>
  DeliveryChat.init({
    appId: "your-app-uuid",
    apiBaseUrl: "https://api.yourcompany.deliverychat.com",
...
| `apiBaseUrl` | `string` | No | API base URL (e.g. `https://api.yourcompany.deliverychat.com`). Defaults to current origin |
...
When `apiBaseUrl` is provided, the widget fetches configuration from:

```
GET {apiBaseUrl}/v1/widget/settings/:appId
```
