...
export function mapAiErrorToResponse(c: Context, error: unknown): Response | null {
  if (error instanceof AITimeoutError) {
    return jsonError(c, 504 as 500, "ai_timeout", "AI provider timed out. Please try again.");
  }
  if (error instanceof AIProviderRateLimitError) {
    return jsonError(c, 503 as 500, "ai_provider_busy", "AI provider is temporarily busy. Please try again.");
  }
  ...
  if (error instanceof AIProviderError) {
    return jsonError(c, 502 as 500, "ai_provider_unavailable", "AI provider is currently unavailable. Please try again.");
  }
  return null;
}
